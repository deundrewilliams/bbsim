...
class VetoPlayers(models.Model):
    ...
    participants = models.ManyToManyField('Houseguest', related_name="veto_player_participants")
    picked = models.ManyToManyField('Houseguest', default=[], related_name="picked_players")
    ...
    def pick_players(self):
        ...
        eligible = list(filter(lambda x: x not in list(self.nominees.all()) and x != self.hoh, list(self.participants.all())))
